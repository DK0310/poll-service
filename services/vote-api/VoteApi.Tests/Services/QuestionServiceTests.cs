...
public class QuestionServiceTests
{
    ...
    public async Task Upvote_IncrementsCount()
    {
        ...
        var result = await _sut.UpvoteAsync("p", q.Id);
    ...
    public async Task Upvote_ReturnsFailure_WhenNotFound()
    {
        ...
        var result = await _sut.UpvoteAsync("p", Guid.NewGuid());

        Assert.False(result.IsSuccess);
    }

    [Fact]
    public async Task TogglePin_FlipsPinnedState()
    {
        var q = new Question { PollCode = "p", Text = "Q", IsPinned = false };
        _repo.Setup(r => r.GetByIdAsync(q.Id)).ReturnsAsync(q);

        var result = await _sut.TogglePinAsync("p", q.Id);
